...
export default function Page() {
  ...
  const [
    ...
    getCategories,
  ] = useNetworkingFunction(catalogs.getCatalog.bind(catalogs), true);
  const [{}, updateCategories] = useNetworkingFunction(
    catalogs.updateCategories.bind(catalogs),
    false
  );

  const [itemsState, setItemsState] = useState<Item[]>([]);
  const [{}, getItemsInCategory] = useNetworkingFunction(
    catalogs.getItemsInCategory.bind(catalogs),
    false
  );
  const [{}, updateItems] = useNetworkingFunction(
    catalogs.updateItems.bind(catalogs),
    false
  );

  const [variationsState, setVariationsState] = useState<Variation[]>([]);
  const [{}, getVariationsForItem] = useNetworkingFunction(
    catalogs.getVariationsForItem.bind(catalogs),
    false
  );
  const [{}, updateVariation] = useNetworkingFunction(
          ...
          <Grid item xs={12} sm={12} md={6}>
            ...
            >
              {preloading ? (
                ...
                <Typography variant="h5">Prepare your catalog</Typography>
              )}
